  ...
  Basemodel.observe("access", async function(ctx) {
    ...
    for (var i = 1; i < roleIds.length; i++) {
      ...
      if (roleIds[i].length > 0) {
        var roles = await AccessRole.find({
          where: { id: { inq: roleIds[i] } }
        });
        for (var j = 0; j < roles.length; j++) {
          var rights = await roles[j].accessRights({
            where: { model: ctx.Model.name }
          });
          if (rights && rights.length > 0) {
            ...
            var groupUsers = await AccessGroupUser.find({
              where: {
                and: [
                  { tier: { lt: i + 2 } },
                  { accessGroupId: groupIds[i][j] }
                ]
              }
            });
            var ownerIds = groupUsers.map(e => {
              return e.userId;
            });
        ...
        if (isCompany) {
          var groupUsers = await AccessGroupUser.find({
            where: {
              and: [{ tier: { lt: i + 2 } }, { companyId: user.companyId }]
            }
          });
          var ownerIds = await groupUsers.map(e => {
            return e.userId;
          });
          allOwnerIds = allOwnerIds.concat(ownerIds);
        }
